  ...
  for (const id of Object.keys(actual)) {
    if (!expectedSet.has(id)) {
      errors.push({
        type: 'schema',
        item: id,
        message: `${label} contains unknown identifier ${id}.`,
      });
    }
  }
}
...
function parseEvidenceLines(section: string | undefined): Map<string, string> {
  ...
  for (const match of section.matchAll(
    /^-\s+([a-z][a-z0-9_-]*):\s+(PASS|FAIL|UNAVAILABLE)\s*$/gm,
  )) {
    const name = match[1];
  ...
  for (const [label, ids] of [
    ...
    for (const id of duplicates(ids)) {
      errors.push({
        type: 'schema',
        item: id,
        message: `Backlog contains duplicate ${label} identifier ${id}.`,
      });
    }
  }
...
function main(): void {
  ...
  try {
    ...
    console.error(
      'Failed to parse planning files:',
