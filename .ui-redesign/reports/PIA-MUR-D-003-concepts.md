...
- **T4 (Workspace switcher placement) = A** — top-left avatar in header (iOS Mail pattern). Calm and Stream use the header avatar; Workspace adds an *additional* 4th tab as a peer, but the avatar is still present in the header (the tab is additive, not a replacement).
...
- **Backend:** None.
- **API:** None. (The `POST /v1/me/active-workspace` endpoint flagged in `PIA-MUR-D-003b` §"Backend, API, data, and route effects" is *out of redesign scope* per `PIA-MUR-D-002` §7 item 1; the design contract will surface this gap but the implementation contract will not implement the endpoint.)
