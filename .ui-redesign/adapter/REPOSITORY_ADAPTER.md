...
- **PWA implementation:** **Implemented** (PIA-MUR-D-004-IMPL commit 8 + follow-up `e1d219a`). `manifest.webmanifest`, `sw.js`, `icon-192.png`, `icon-512.png`, `apple-touch-icon.png`, `maskable-icon-512.png` in `apps/web/public/`. Service worker path is `sw.js` (not `service-worker.js`). `viewport-fit=cover`, `theme-color`, `apple-mobile-web-app-capable`, `apple-mobile-web-app-status-bar-style` meta tags in page shell. `safe-area-inset` CSS applied. PWA static-asset route at `apps/api/src/routes/pwa-assets.ts`. Network-required PWA (no offline scope). Device validation deferred — no real iPhone install confirmed.
- **Existing browser automation:** **None in production CI.** `.github/workflows/ci.yaml` covers format, lint, typecheck, unit tests, build, secret scan, dependency audit only. **Local-host automation (dev-only, PIA-MUR-D-016 approved 2026-06-14):** Playwright 1.49.0 + axe-core 4.10.0 as `apps/web/devDependencies`; Chromium + WebKit browser binaries downloaded to `~/.cache/ms-playwright/` (~230 MB total, NOT in the repo); harness at `apps/web/test/preflight/` runs against the live prototype (`http://100.81.83.98:8000/`) and the live API (`http://100.81.83.98:3000/`) via Tailscale. CI integration is **out of scope** for the redesign (future ADR).
